...
export const migration003: Migration = {
  ...
  async up() {
    Logger.info('Running migration 003: Restructure job schema with new fields');
    ...
    Logger.info(`${jobCount} jobs in database - new structure will apply to future jobs`);

    Logger.info('Migration 003 completed');
  },

  async down() {
    Logger.info('Rolling back migration 003 (No-op - schema changes are backward compatible)');
